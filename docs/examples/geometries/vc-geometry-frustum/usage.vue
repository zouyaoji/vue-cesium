...
import type { VcGeometryFrustumOutlineRef, VcGeometryFrustumRef, VcGeometryInstanceProps } from 'vue-cesium'
import type { VcAppearance, VcPickEvent, VcReadyObject } from 'vue-cesium/es/utils/types'
import { onMounted, ref, shallowRef } from 'vue'

const geometryRef = shallowRef<VcGeometryFrustumRef>()
const geometryOutlineRef = shallowRef<VcGeometryFrustumOutlineRef>()
...
const frustum = shallowRef<Cesium.PerspectiveFrustum>()
const vertexFormat = shallowRef<Cesium.VertexFormat>()
const orientation = shallowRef<Cesium.Quaternion>()
...
function onViewerReady({ Cesium, viewer }: VcReadyObject) {
  const { ColorGeometryInstanceAttribute, PerInstanceColorAppearance, PerspectiveFrustum, Quaternion } = Cesium
  attributes.value = {
  ...
  orientation.value = new Quaternion(0, 0, 0, 1)
}
...
<template>
  <el-row class="demo-viewer demo-vc-geometry-frustum">
    <vc-viewer @ready="onViewerReady">
      ...
      <vc-primitive v-if="outline && frustum" :appearance="appearance" @click="onClicked">
