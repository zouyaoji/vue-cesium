...
`vc-navigation-sm` 组件用于加载导航罗盘部件——超图样式。**注意：** 使用此组件需要引入 `import 'vue-cesium/lib/vc-navigation-sm.css'`，需要`css-loader`。
  ...
  <template>
    <div class="viewer">
      <vc-viewer :timeline="timeline" @ready="ready" >
        <vc-navigation-sm :options="options"></vc-navigation-sm>
      </vc-viewer>
      <div class="demo-tool">
        <span>罗盘部件</span>
