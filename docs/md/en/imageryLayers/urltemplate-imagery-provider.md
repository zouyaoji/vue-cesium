...
`urltemplate-imagery-provider`It is convenient for users to request image layers through a contracted URL template. For example, loading AMap, Tencent and other image services, URLs are a fixed specification, which can be easily implemented through this component.

## Examples

### add a UrlTemplateImageryProvider layer to viewer
