define([
  ...
  './Math-2dbd6b93',
  './Plane-900aa728'
], function (exports, Transforms, Matrix2, Matrix3, Check, defaultValue, EllipsoidTangentPlane, Math$1, Plane) {
  'use strict'
  ...
  OrientedBoundingBox.fromPoints = function (positions, result) {
    ...
    const eigenDecomposition = Matrix3.Matrix3.computeEigenDecomposition(covarianceMatrix, scratchEigenResult)
  ...
  OrientedBoundingBox.fromRectangle = function (rectangle, minimumHeight, maximumHeight, ellipsoid, result) {
    ...
    if (rectangle.width <= Math$1.CesiumMath.PI) {
      ...
      const tangentPointCartographic = Matrix2.Rectangle.center(rectangle, scratchRectangleCenterCartographic)
      ...
      const perimeterCartographicNC = Matrix3.Cartographic.fromRadians(lonCenter, rectangle.north, maximumHeight, scratchPerimeterCartographicNC)
      const perimeterCartographicNW = Matrix3.Cartographic.fromRadians(rectangle.west, rectangle.north, maximumHeight, scratchPerimeterCartographicNW)
      const perimeterCartographicCW = Matrix3.Cartographic.fromRadians(rectangle.west, latCenter, maximumHeight, scratchPerimeterCartographicCW)
      const perimeterCartographicSW = Matrix3.Cartographic.fromRadians(rectangle.west, rectangle.south, maximumHeight, scratchPerimeterCartographicSW)
      const perimeterCartographicSC = Matrix3.Cartographic.fromRadians(lonCenter, rectangle.south, maximumHeight, scratchPerimeterCartographicSC)
    ...
    const centerLongitude = Matrix2.Rectangle.center(rectangle, scratchRectangleCenterCartographic).longitude

    // Plane is located at the rectangle's center longitude and the rectangle's latitude that is closest to the equator. It rotates around the Z axis.
    // This results in a better fit than the obb approach for smaller rectangles, which orients with the rectangle's center normal.
    const planeOrigin = Matrix3.Cartesian3.fromRadians(centerLongitude, latitudeNearestToEquator, maximumHeight, ellipsoid, scratchPlaneOrigin)
    planeOrigin.z = 0.0 // center the plane on the equator to simpify plane normal calculation
    const isPole = Math.abs(planeOrigin.x) < Math$1.CesiumMath.EPSILON10 && Math.abs(planeOrigin.y) < Math$1.CesiumMath.EPSILON10
    const planeNormal = !isPole ? Matrix3.Cartesian3.normalize(planeOrigin, scratchPlaneNormal) : Matrix3.Cartesian3.UNIT_X
    ...
    const farZ = Matrix3.Cartesian3.fromRadians(rectangle.east, latitudeNearestToEquator, maximumHeight, ellipsoid, scratchZ)
  ...
  OrientedBoundingBox.computeTransformation = function (box, result) {
    ...
    const rotationScale = Matrix3.Matrix3.multiplyByUniformScale(box.halfAxes, 2.0, scratchRotationScale)
