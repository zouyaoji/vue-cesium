...
define([
  ...
  './Plane-4f333bc4'
], function (exports, Matrix2, when, RuntimeError, EllipsoidGeodesic, EllipsoidRhumbLine, IntersectionTests, ComponentDatatype, Plane) {
  'use strict'
  ...
  PolylinePipeline.wrapLongitude = function (positions, modelMatrix) {
    ...
    if (when.defined(positions) && positions.length > 0) {
      ...
      var origin = Matrix2.Matrix4.multiplyByPoint(inverseModelMatrix, Matrix2.Cartesian3.ZERO, wrapLongitudeOrigin)
      var xzNormal = Matrix2.Cartesian3.normalize(
        Matrix2.Matrix4.multiplyByPointAsVector(inverseModelMatrix, Matrix2.Cartesian3.UNIT_Y, wrapLongitudeXZNormal),
      ...
      var yzNormal = Matrix2.Cartesian3.normalize(
        Matrix2.Matrix4.multiplyByPointAsVector(inverseModelMatrix, Matrix2.Cartesian3.UNIT_X, wrapLongitudeYZNormal),
      ...
      for (var i = 1; i < length; ++i) {
        ...
        if (Plane.Plane.getPointDistance(yzPlane, prev) < 0.0 || Plane.Plane.getPointDistance(yzPlane, cur) < 0.0) {
          ...
          if (when.defined(intersection)) {
            ...
            var offset = Matrix2.Cartesian3.multiplyByScalar(xzNormal, 5.0e-9, wrapLongitudeOffset)
            if (Plane.Plane.getPointDistance(xzPlane, prev) < 0.0) {
              Matrix2.Cartesian3.negate(offset, offset)
            }

            cartesians.push(Matrix2.Cartesian3.add(intersection, offset, new Matrix2.Cartesian3()))
