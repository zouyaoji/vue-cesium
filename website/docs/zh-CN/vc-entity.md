...
`vc-entity` 组件用于加载 `Cesium.Entity` 实例。需要作为 `vc-viewer`, `vc-datasource-custom`, `vc-datasource-geojson`, `vc-datasource-kml`, `vc-datasource-czml` 的子组件才能正常加载。使用时可以直接指定 `vc-entity` 的各个 `graphic` 属性，也用 VueCesium 提供的 `vc-graphics-xxx` 系列组件作为 `vc-entity` 子组件挂载各个 `graphic`，但一个实体下同类型 `grahpic` 只能挂载一个。如果需要加载大量实体实例集合，建议添加到数据源组件下。
