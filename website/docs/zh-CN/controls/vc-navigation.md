...
`vc-navigation` 导航组件，主要包括 2 个部分，（1）罗盘、缩放、其他悬浮按钮；（2）位置和距离比例尺工具栏。**注意：** 该需要引入样式文件: `import 'vue-cesium/lib/theme-default/index.css';`
...
按需定制导航组件。可用的组件有: `vc-compass`、`vc-zoom-control`、`vc-print`、`vc-mylocation`、`vc-location-bar`、`vc-distance-legend`。
...
| 属性名       | 类型          | 默认值                      | 描述                                                                                |
| ------------ | ------------- | --------------------------- | ----------------------------------------------------------------------------------- |
| position     | String        | `top-right`                 | `optional` 指定导航组件位置。                                                       |
...
| locationEvt       |      | 操作定位控件时触发。         |
| printEvt          |      | 操作打印控件时触发。         |
| locationBarEvt    |      | 位置控件相关参数改变时触发。 |
| distanceLegendEvt |      | 距离比例尺改变时触发。       |

### VcCompass 属性

| 属性名                 | 类型   | 默认值      | 描述                                                     |
| ---------------------- | ------ | ----------- | -------------------------------------------------------- |
| position               | String | `top-right` | `optional` 指定罗盘位置。（单独使用时有效）              |
| offset                 | Array  | `[0, 0]`    | `optional` 指定罗盘基于位置的偏移量。 （单独使用时有效） |
...
| 属性名               | 类型    | 默认值                                 | 描述                                                         |
| -------------------- | ------- | -------------------------------------- | ------------------------------------------------------------ |
| position             | String  | `'top-right'`                          | `optional` 指定缩放控件位置。（单独使用时有效）              |
| offset               | Array   | `[0, 0]`                               | `optional` 指定缩放控件基于位置的偏移量。 （单独使用时有效） |
...
| direction            | String  | `'vertical'`                           | `optional` 指定缩放控件方向。可选`'vertical', 'horizontal'`  |
...
| 属性名                | 类型    | 默认值               | 描述                                                         |
| --------------------- | ------- | -------------------- | ------------------------------------------------------------ |
| position              | String  | `'top-right'`        | `optional` 指定打印控件位置。（单独使用时有效）              |
| offset                | Array   | `[0, 0]`             | `optional` 指定打印控件基于位置的偏移量。 （单独使用时有效） |
...
| 属性名       | 类型                  | 默认值                 | 描述                                                         |
| ------------ | --------------------- | ---------------------- | ------------------------------------------------------------ |
| position     | String                | `'top-right'`          | `optional` 指定定位按钮位置。（单独使用时有效）              |
| offset       | Array                 | `[0, 0]`               | `optional` 指定定位按钮基于位置的偏移量。 （单独使用时有效） |
...
| 属性名 | 类型 | 默认值 | 描述 |
| ------------------- | ------- | ---------------- | -------------------------------------------------------------- |
| position | String | `'bottom-right'` | `optional` 指定信息栏控件位置。（单独使用时有效） |
| offset | Array | `[0, 0]` | `optional` 指定信息栏控件基于位置的偏移量。 （单独使用时有效） |
| gridFileUrl | String | `https://zouyaoji.top/vue-cesium/statics/SampleData/WW15MGH.DAC`| `optional` 指定鼠标拾取高度模型，用这个能提高获取的高度精度。 |
...
| 事件名         | 参数 | 描述                       |
| -------------- | ---- | -------------------------- |
| locationBarEvt |      | 信息栏相关参数改变时触发。 |

### VcDistanceLegend 属性

| 属性名        | 类型   | 默认值           | 描述                                                               |
| ------------- | ------ | ---------------- | ------------------------------------------------------------------ |
| position      | String | `'bottom-right'` | `optional` 指定距离比例尺控件位置。（单独使用时有效）              |
| offset        | Array  | `[0, 0]`         | `optional` 指定距离比例尺控件基于位置的偏移量。 （单独使用时有效） |
