define([
  ...
  function computeTopBottomAttributes(positions, options, extrude) {
    ...
    if (stRotation !== 0) {
      let rotation = Transforms.Quaternion.fromAxisAngle(geodeticNormal, stRotation, quaternionScratch)
    ...
    const minTexCoord = Matrix2.Cartesian2.fromElements(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, scratchMinTexCoord)
    const maxTexCoord = Matrix2.Cartesian2.fromElements(Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY, scratchMaxTexCoord)
    ...
    for (let i = 0; i < length; i += 3) {
      ...
      if (vertexFormat.st) {
        const rotatedPoint = Matrix3.Matrix3.multiplyByVector(textureMatrix, position, scratchCartesian2)
    ...
    if (vertexFormat.position) {
      const finalPositions = EllipseGeometryLibrary.EllipseGeometryLibrary.raisePositionsToHeight(positions, options, extrude)
  ...
  function computeEllipse(options) {
    ...
    const boundingSphere = new Transforms.BoundingSphere(boundingSphereCenter, options.semiMajorAxis)
    const cep = EllipseGeometryLibrary.EllipseGeometryLibrary.computeEllipsePositions(options, true, false)
  ...
  function computeWallAttributes(positions, options) {
    ...
    const geodeticNormal = ellipsoid.scaleToGeodeticSurface(center, scratchCartesian1)
    ellipsoid.geodeticSurfaceNormal(geodeticNormal, geodeticNormal)
    const rotation = Transforms.Quaternion.fromAxisAngle(geodeticNormal, stRotation, quaternionScratch)
  ...
  function computeExtrudedEllipse(options) {
    ...
    const cep = EllipseGeometryLibrary.EllipseGeometryLibrary.computeEllipsePositions(options, true, true)
    const positions = cep.positions
    const numPts = cep.numPts
    ...
    const topBottomIndices = IndexDatatype.IndexDatatype.createTypedArray((posLength * 2) / 3, indices)
    ...
    const wallIndices = IndexDatatype.IndexDatatype.createTypedArray((outerPositions.length * 2) / 3, indices)
  ...
  EllipseGeometry.createGeometry = function (ellipseGeometry) {
    ...
    if (extrude) {
      ...
      if (defaultValue.defined(ellipseGeometry._offsetAttribute)) {
        ...
        const offsetValue = ellipseGeometry._offsetAttribute === GeometryOffsetAttribute.GeometryOffsetAttribute.NONE ? 0 : 1
