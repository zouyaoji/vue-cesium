...
define([
...
], function (exports, Transforms, Matrix2, RuntimeError, ComponentDatatype, when, GeometryAttribute, GeometryAttributes, Plane, VertexFormat) {
  'use strict'
  ...
  CullingVolume.fromBoundingSphere = function (boundingSphere, result) {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      plane1.w = -Matrix2.Cartesian3.dot(Matrix2.Cartesian3.negate(faceNormal, scratchPlaneNormal), scratchPlaneCenter)
  ...
  function update$3(frustum) {
    ...
    if (
      ...
      throw new RuntimeError.DeveloperError('right, left, top, bottom, near, or far parameters are not set.')
    }
    //>>includeEnd('debug');
    ...
    if (
      ...
      if (frustum.near <= 0 || frustum.near > frustum.far) {
        throw new RuntimeError.DeveloperError('near must be greater than zero and less than far.')
      }
      //>>includeEnd('debug');
  ...
  OrthographicOffCenterFrustum.prototype.getPixelDimensions = function (drawingBufferWidth, drawingBufferHeight, distance, pixelRatio, result) {
    ...
    if (!when.defined(drawingBufferWidth) || !when.defined(drawingBufferHeight)) {
      throw new RuntimeError.DeveloperError('Both drawingBufferWidth and drawingBufferHeight are required.')
    }
  ...
  OrthographicOffCenterFrustum.prototype.equalsEpsilon = function (other, relativeEpsilon, absoluteEpsilon) {
    return (
      ...
      (when.defined(other) &&
        other instanceof OrthographicOffCenterFrustum &&
        ComponentDatatype.CesiumMath.equalsEpsilon(this.right, other.right, relativeEpsilon, absoluteEpsilon) &&
        ComponentDatatype.CesiumMath.equalsEpsilon(this.left, other.left, relativeEpsilon, absoluteEpsilon) &&
        ComponentDatatype.CesiumMath.equalsEpsilon(this.top, other.top, relativeEpsilon, absoluteEpsilon) &&
        ComponentDatatype.CesiumMath.equalsEpsilon(this.bottom, other.bottom, relativeEpsilon, absoluteEpsilon) &&
        ComponentDatatype.CesiumMath.equalsEpsilon(this.near, other.near, relativeEpsilon, absoluteEpsilon) &&
        ComponentDatatype.CesiumMath.equalsEpsilon(this.far, other.far, relativeEpsilon, absoluteEpsilon))
    )
  }
  ...
  function update$2(frustum) {
    ...
    if (!when.defined(frustum.width) || !when.defined(frustum.aspectRatio) || !when.defined(frustum.near) || !when.defined(frustum.far)) {
      throw new RuntimeError.DeveloperError('width, aspectRatio, near, or far parameters are not set.')
    }
    //>>includeEnd('debug');
  ...
  OrthographicFrustum.prototype.equalsEpsilon = function (other, relativeEpsilon, absoluteEpsilon) {
    ...
    return (
      ComponentDatatype.CesiumMath.equalsEpsilon(this.width, other.width, relativeEpsilon, absoluteEpsilon) &&
      ComponentDatatype.CesiumMath.equalsEpsilon(this.aspectRatio, other.aspectRatio, relativeEpsilon, absoluteEpsilon) &&
  ...
  function update(frustum) {
    ...
    if (!when.defined(frustum.fov) || !when.defined(frustum.aspectRatio) || !when.defined(frustum.near) || !when.defined(frustum.far)) {
      throw new RuntimeError.DeveloperError('fov, aspectRatio, near, or far parameters are not set.')
    }
    //>>includeEnd('debug');
  ...
  PerspectiveFrustum.prototype.equalsEpsilon = function (other, relativeEpsilon, absoluteEpsilon) {
    ...
    return (
      ComponentDatatype.CesiumMath.equalsEpsilon(this.fov, other.fov, relativeEpsilon, absoluteEpsilon) &&
      ComponentDatatype.CesiumMath.equalsEpsilon(this.aspectRatio, other.aspectRatio, relativeEpsilon, absoluteEpsilon) &&
      this._offCenterFrustum.equalsEpsilon(other._offCenterFrustum, relativeEpsilon, absoluteEpsilon)
    )
  }
  ...
  function FrustumGeometry(options) {
     ...
     */
    this.packedLength =
      2 + frustumPackedLength + Matrix2.Cartesian3.packedLength + Transforms.Quaternion.packedLength + VertexFormat.VertexFormat.packedLength
  }
  ...
  FrustumGeometry._computeNearFarPlanes = function (origin, orientation, frustumType, frustum, positions, xDirection, yDirection, zDirection) {
    var rotationMatrix = Matrix2.Matrix3.fromQuaternion(orientation, scratchRotationMatrix)
