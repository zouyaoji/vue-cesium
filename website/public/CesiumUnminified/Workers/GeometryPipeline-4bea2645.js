...
define([
  ...
  GeometryPipeline.toWireframe = function (geometry) {
    ...
    if (when.defined(indices)) {
      switch (geometry.primitiveType) {
        ...
        //>>includeStart('debug', pragmas.debug);
        default:
          throw new RuntimeError.DeveloperError('geometry.primitiveType must be TRIANGLES, TRIANGLE_STRIP, or TRIANGLE_FAN.')
        //>>includeEnd('debug');
      }
  ...
  GeometryPipeline.fitToUnsignedShortIndices = function (geometry) {
    ...
    if (when.defined(geometry.indices) && numberOfVertices >= ComponentDatatype.CesiumMath.SIXTY_FOUR_KILOBYTES) {
      var oldToNewIndex = []
      var newIndices = []
      var currentIndex = 0
      ...
      for (var j = 0; j < numberOfIndices; j += indicesPerPrimitive) {
        ...
        if (currentIndex + indicesPerPrimitive >= ComponentDatatype.CesiumMath.SIXTY_FOUR_KILOBYTES) {
          geometries.push(
  ...
  GeometryPipeline.projectTo2D = function (geometry, attributeName, attributeName3D, attributeName2D, projection) {
    ...
    if (!when.defined(geometry.attributes[attributeName])) {
      throw new RuntimeError.DeveloperError('geometry must have attribute matching the attributeName argument: ' + attributeName + '.')
    }
    if (geometry.attributes[attributeName].componentDatatype !== ComponentDatatype.ComponentDatatype.DOUBLE) {
      throw new RuntimeError.DeveloperError('The attribute componentDatatype must be ComponentDatatype.DOUBLE.')
    }
    //>>includeEnd('debug');
    ...
    for (var i = 0; i < values3D.length; i += 3) {
      ...
      if (!when.defined(lonLat)) {
        throw new RuntimeError.DeveloperError('Could not project point (' + value.x + ', ' + value.y + ', ' + value.z + ') to 2D.')
      }
      //>>includeEnd('debug');
  ...
  function transformVector(matrix, attribute) {
    if (when.defined(attribute)) {
      ...
      for (var i = 0; i < length; i += 3) {
        ...
        scratchCartesian3 = Matrix2.Cartesian3.normalize(scratchCartesian3, scratchCartesian3)
        Matrix2.Cartesian3.pack(scratchCartesian3, values, i)
      }
    }
  }
  ...
  function combineGeometries(instances, propertyName) {
    ...
    for (i = 1; i < length; ++i) {
      ...
      if (when.defined(instances[i][propertyName].indices) !== haveIndices) {
        throw new RuntimeError.DeveloperError('All instance geometries must have an indices or not have one.')
      }
      if (instances[i][propertyName].primitiveType !== primitiveType) {
        throw new RuntimeError.DeveloperError('All instance geometries must have the same primitiveType.')
      }
    }
    //>>includeEnd('debug');
    ...
    if (when.defined(center)) {
      ...
      for (i = 0; i < length; ++i) {
        bs = instances[i][propertyName].boundingSphere
        var tempRadius = Matrix2.Cartesian3.magnitude(Matrix2.Cartesian3.subtract(bs.center, center, tempScratch)) + bs.radius
  ...
  GeometryPipeline.combineInstances = function (instances) {
    ...
    if (!when.defined(instances) || instances.length < 1) {
      throw new RuntimeError.DeveloperError('instances is required and must have length greater than zero.')
    }
    //>>includeEnd('debug');
  ...
  GeometryPipeline.computeNormal = function (geometry) {
    ...
    if (!when.defined(geometry.attributes.position) || !when.defined(geometry.attributes.position.values)) {
      throw new RuntimeError.DeveloperError('geometry.attributes.position.values is required.')
    }
    if (!when.defined(geometry.indices)) {
      throw new RuntimeError.DeveloperError('geometry.indices is required.')
    }
    if (geometry.indices.length < 2 || geometry.indices.length % 3 !== 0) {
      throw new RuntimeError.DeveloperError('geometry.indices length must be greater than 0 and be a multiple of 3.')
    }
    if (geometry.primitiveType !== GeometryAttribute.PrimitiveType.TRIANGLES) {
      throw new RuntimeError.DeveloperError('geometry.primitiveType must be PrimitiveType.TRIANGLES.')
    }
    //>>includeEnd('debug');
    ...
    for (i = 0; i < numVertices; i++) {
      ...
      if (vertexNormalData.count > 0) {
        for (j = 0; j < vertexNormalData.count; j++) {
          Matrix2.Cartesian3.add(normal, normalsPerTriangle[normalIndices[vertexNormalData.indexOffset + j]], normal)
        }

        // We can run into an issue where a vertex is used with 2 primitives that have opposite winding order.
        if (Matrix2.Cartesian3.equalsEpsilon(Matrix2.Cartesian3.ZERO, normal, ComponentDatatype.CesiumMath.EPSILON10)) {
          Matrix2.Cartesian3.clone(normalsPerTriangle[normalIndices[vertexNormalData.indexOffset]], normal)
        }
      }
  ...
  function indexTriangles(geometry) {
    ...
    if (numberOfVertices % 3 !== 0) {
      throw new RuntimeError.DeveloperError('The number of vertices must be a multiple of three.')
    }
    //>>includeEnd('debug');
  ...
  function getXZIntersectionOffsetPoints(p, p1, u1, v1) {
    Matrix2.Cartesian3.add(p, Matrix2.Cartesian3.multiplyByScalar(Matrix2.Cartesian3.subtract(p1, p, c3), p.y / (p.y - p1.y), c3), u1)
  ...
  var interpolateAndPackCartesian3 = generateBarycentricInterpolateFunction(Matrix2.Cartesian3, 3)
  ...
  function computeTriangleAttributes(
    ...
    if (when.defined(extrudeDirections)) {
      ...
      Matrix2.Cartesian3.pack(direction, currentAttributes.extrudeDirection.values, insertedIndex * 3)
    }
  ...
  function splitLongitudeLines(instance) {
    ...
    for (i = 0; i < length; i += 2) {
      ...
      if (when.defined(intersection)) {
        ...
        var offset = Matrix2.Cartesian3.multiplyByScalar(Matrix2.Cartesian3.UNIT_Y, 5.0 * ComponentDatatype.CesiumMath.EPSILON9, offsetScratch)
        if (p0.y < 0.0) {
        ...
        var offsetPoint = Matrix2.Cartesian3.add(intersection, offset, offsetPointScratch)
  ...
  function splitLongitudePolyline(instance) {
    ...
    for (i = 0; i < length; i += 4) {
      ...
      if (when.defined(intersection)) {
        ...
        // move point on the xz-plane slightly away from the plane
        var offset = Matrix2.Cartesian3.multiplyByScalar(Matrix2.Cartesian3.UNIT_Y, offsetScalar, cartesian3Scratch5)
        if (p0.y < 0.0) {
          Matrix2.Cartesian3.negate(offset, offset)
          p0Attributes = westGeometry.attributes
          p0Indices = westGeometry.indices
        ...
        var offsetPoint = Matrix2.Cartesian3.add(intersection, offset, cartesian3Scratch6)
        ...
        var t = Matrix2.Cartesian3.magnitudeSquared(Matrix2.Cartesian3.subtract(intersection, p0, cartesian3Scratch3))
        t /= Matrix2.Cartesian3.magnitudeSquared(Matrix2.Cartesian3.subtract(p2, p0, cartesian3Scratch3))
...
//# sourceMappingURL=GeometryPipeline-4bea2645.js.map
