define([
    ...
    if (defaultValue.defined(indices)) {
      ...
      for (const property in attributes) {
        if (attributes.hasOwnProperty(property) && defaultValue.defined(attributes[property]) && defaultValue.defined(attributes[property].values)) {
          ...
          const elementsOut = ComponentDatatype.ComponentDatatype.createTypedArray(attribute.componentDatatype, nextIndex * numComponents)
  ...
  function combineGeometries(instances, propertyName) {
    ...
    if (haveIndices) {
      ...
      const destIndices = IndexDatatype.IndexDatatype.createTypedArray(numberOfVertices, numberOfIndices)
    ...
    if (defaultValue.defined(center)) {
      ...
      for (i = 0; i < length; ++i) {
        bs = instances[i][propertyName].boundingSphere
        const tempRadius = Matrix3.Cartesian3.magnitude(Matrix3.Cartesian3.subtract(bs.center, center, tempScratch)) + bs.radius
  ...
  function indexTriangleFan(geometry) {
    ...
    const indices = IndexDatatype.IndexDatatype.createTypedArray(numberOfVertices, (numberOfVertices - 2) * 3)
    indices[0] = 1
    indices[1] = 0
    indices[2] = 2
  ...
  function indexLineStrip(geometry) {
    ...
    const indices = IndexDatatype.IndexDatatype.createTypedArray(numberOfVertices, (numberOfVertices - 1) * 2)
    indices[0] = 0
    indices[1] = 1
    let indicesIndex = 2
  ...
  function indexLineLoop(geometry) {
    ...
    const indices = IndexDatatype.IndexDatatype.createTypedArray(numberOfVertices, numberOfVertices * 2)
  ...
  function splitLongitudeTriangles(instance) {
    ...
    const bitangents = defaultValue.defined(attributes.bitangent) ? attributes.bitangent.values : undefined
    const tangents = defaultValue.defined(attributes.tangent) ? attributes.tangent.values : undefined
    const texCoords = defaultValue.defined(attributes.st) ? attributes.st.values : undefined
    const extrudeDirections = defaultValue.defined(attributes.extrudeDirection) ? attributes.extrudeDirection.values : undefined
    const applyOffset = defaultValue.defined(attributes.applyOffset) ? attributes.applyOffset.values : undefined
    const indices = geometry.indices
  ...
  function splitLongitudeLines(instance) {
    ...
    for (i = 0; i < length; i += 2) {
      ...
      const intersection = IntersectionTests.IntersectionTests.lineSegmentPlane(p0, p1, xzPlane, p2Scratch)
      if (defaultValue.defined(intersection)) {
        // move point on the xz-plane slightly away from the plane
        const offset = Matrix3.Cartesian3.multiplyByScalar(Matrix3.Cartesian3.UNIT_Y, 5.0 * Math$1.CesiumMath.EPSILON9, offsetScratch)
        if (p0.y < 0.0) {
  ...
  function splitLongitudePolyline(instance) {
    ...
    for (i = 0; i < length; i += 4) {
      ...
      const intersection = IntersectionTests.IntersectionTests.lineSegmentPlane(p0, p2, xzPlane, cartesian3Scratch4)
      if (defaultValue.defined(intersection)) {
        intersectionFound = true

        // move point on the xz-plane slightly away from the plane
        const offset = Matrix3.Cartesian3.multiplyByScalar(Matrix3.Cartesian3.UNIT_Y, offsetScalar, cartesian3Scratch5)
        if (p0.y < 0.0) {
          Matrix3.Cartesian3.negate(offset, offset)
          p0Attributes = westGeometry.attributes
          p0Indices = westGeometry.indices
        ...
        let t = Matrix3.Cartesian3.magnitudeSquared(Matrix3.Cartesian3.subtract(intersection, p0, cartesian3Scratch3))
