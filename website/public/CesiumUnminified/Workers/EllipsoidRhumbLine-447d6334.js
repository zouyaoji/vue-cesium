...
define(['exports', './Matrix2-91d5b6af', './RuntimeError-346a3079', './when-4bbc8319', './ComponentDatatype-f194c48b'], function (
  exports,
  Matrix2,
  RuntimeError,
  when,
  ...
  function calculateSigma(ellipticity, latitude) {
    ...
    return Math.log(Math.tan(0.5 * (ComponentDatatype.CesiumMath.PI_OVER_TWO + latitude))) - (ellipticity / 2.0) * Math.log((1 + eSinL) / (1 - eSinL))
  }
  ...
  function calculateHeading(ellipsoidRhumbLine, firstLongitude, firstLatitude, secondLongitude, secondLatitude) {
    ...
    return Math.atan2(ComponentDatatype.CesiumMath.negativePiToPi(secondLongitude - firstLongitude), sigma2 - sigma1)
  }
  ...
  function calculateArcLength(ellipsoidRhumbLine, major, minor, firstLongitude, firstLatitude, secondLongitude, secondLatitude) {
    ...
    if (
      ComponentDatatype.CesiumMath.equalsEpsilon(Math.abs(heading), ComponentDatatype.CesiumMath.PI_OVER_TWO, ComponentDatatype.CesiumMath.EPSILON8)
    ) {
      //If heading is close to 90 degrees
      if (major === minor) {
        distance = major * Math.cos(firstLatitude) * ComponentDatatype.CesiumMath.negativePiToPi(deltaLongitude)
      } else {
        var sinPhi = Math.sin(firstLatitude)
        distance =
          (major * Math.cos(firstLatitude) * ComponentDatatype.CesiumMath.negativePiToPi(deltaLongitude)) /
  ...
  function computeProperties(ellipsoidRhumbLine, start, end, ellipsoid) {
    var firstCartesian = Matrix2.Cartesian3.normalize(ellipsoid.cartographicToCartesian(start, scratchCart2), scratchCart1)
    var lastCartesian = Matrix2.Cartesian3.normalize(ellipsoid.cartographicToCartesian(end, scratchCart2), scratchCart2)

    //>>includeStart('debug', pragmas.debug);
    RuntimeError.Check.typeOf.number.greaterThanOrEquals(
      'value',
      Math.abs(Math.abs(Matrix2.Cartesian3.angleBetween(firstCartesian, lastCartesian)) - Math.PI),
      0.0125
    )
    //>>includeEnd('debug');
    ...
    ellipsoidRhumbLine._start = Matrix2.Cartographic.clone(start, ellipsoidRhumbLine._start)
  ...
  function interpolateUsingSurfaceDistance(start, heading, distance, major, ellipticity, result) {
    ...
    if (Math.abs(ComponentDatatype.CesiumMath.PI_OVER_TWO - Math.abs(heading)) > ComponentDatatype.CesiumMath.EPSILON8) {
  ...
  EllipsoidRhumbLine.prototype.interpolateUsingSurfaceDistance = function (distance, result) {
    ...
    if (!when.defined(this._distance) || this._distance === 0.0) {
      throw new RuntimeError.DeveloperError('EllipsoidRhumbLine must have distinct start and end set.')
    }
    //>>includeEnd('debug');
  ...
  EllipsoidRhumbLine.prototype.findIntersectionWithLongitude = function (intersectionLongitude, result) {
    ...
    if (ComponentDatatype.CesiumMath.equalsEpsilon(Math.abs(intersectionLongitude), Math.PI, ComponentDatatype.CesiumMath.EPSILON14)) {
    ...
    if (Math.abs(ComponentDatatype.CesiumMath.PI_OVER_TWO - absHeading) <= ComponentDatatype.CesiumMath.EPSILON8) {
      ...
      if (ComponentDatatype.CesiumMath.equalsEpsilon(intersectionLongitude, start.longitude, ComponentDatatype.CesiumMath.EPSILON12)) {
        return undefined
      }

      result.longitude = intersectionLongitude
      result.latitude =
        ComponentDatatype.CesiumMath.PI_OVER_TWO * ComponentDatatype.CesiumMath.sign(ComponentDatatype.CesiumMath.PI_OVER_TWO - heading)
      result.height = 0
      return result
    }
    ...
    var leftComponent =
      Math.tan(0.5 * (ComponentDatatype.CesiumMath.PI_OVER_TWO + phi1)) * Math.exp((intersectionLongitude - start.longitude) / Math.tan(heading))
    ...
    do {
      ...
      newPhi = 2 * Math.atan(leftComponent * Math.pow(numerator / denominator, ellipticity / 2)) - ComponentDatatype.CesiumMath.PI_OVER_TWO
