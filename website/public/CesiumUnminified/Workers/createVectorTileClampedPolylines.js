define([
  ...
  VertexAttributesAndIndices.prototype.addVolume = function (
    ...
    const startEllipsoidNormal = ellipsoid.geodeticSurfaceNormal(position, scratchStartEllipsoidNormal)
    ...
    const startFaceNormal = computeMiteredNormal(preStartRTC, startRTC, endRTC, startEllipsoidNormal, scratchStartFaceNormal)
    const endFaceNormal = computeMiteredNormal(postEndRTC, endRTC, startRTC, endEllipsoidNormal, scratchEndFaceNormal)
  ...
  function createVectorTileClampedPolylines(parameters, transferableObjects) {
    ...
    const positions = decodePositions(uBuffer, vBuffer, heightBuffer, rectangle, minimumHeight, maximumHeight, ellipsoid)
    ...
    for (i = 0; i < countsLength; i++) {
      ...
      for (let j = 0; j < polylineVolumeCount; j++) {
        ...
        const volumeEnd = Matrix3.Cartesian3.unpack(positionsRTC, currentPositionIndex + 3, scratchP1)
        ...
        if (j === 0) {
          ...
          const finalPosition = Matrix3.Cartesian3.unpack(positionsRTC, finalPositionIndex, scratchPrev)
        ...
        if (j === polylineVolumeCount - 1) {
          // Check if this volume is like a loop
          const firstPosition = Matrix3.Cartesian3.unpack(positionsRTC, volumeFirstPositionIndex, scratchNext)
