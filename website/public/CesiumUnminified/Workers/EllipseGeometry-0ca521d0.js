...
define([
  ...
  function computeTopBottomAttributes(positions, options, extrude) {
    ...
    for (var i = 0; i < length; i += 3) {
      ...
      if (vertexFormat.st) {
        var rotatedPoint = Matrix2.Matrix3.multiplyByVector(textureMatrix, position, scratchCartesian2)
      ...
      if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent || shadowVolume) {
        ...
        if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
          if (vertexFormat.tangent || vertexFormat.bitangent) {
            tangent = Matrix2.Cartesian3.normalize(Matrix2.Cartesian3.cross(Matrix2.Cartesian3.UNIT_Z, normal, tangent), tangent)
  ...
  function computeEllipse(options) {
    ...
    boundingSphereCenter = Matrix2.Cartesian3.add(center, boundingSphereCenter, boundingSphereCenter)
  ...
  function computeWallAttributes(positions, options) {
    ...
    for (var i = 0; i < length; i += 3) {
      ...
      var scaledNormal = Matrix2.Cartesian3.multiplyByScalar(normal, height, scratchCartesian4)
      position = Matrix2.Cartesian3.add(position, scaledNormal, position)
      scaledNormal = Matrix2.Cartesian3.multiplyByScalar(normal, extrudedHeight, scaledNormal)
      extrudedPosition = Matrix2.Cartesian3.add(extrudedPosition, scaledNormal, extrudedPosition)
      ...
      if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
        ...
        var next = Matrix2.Cartesian3.fromArray(positions, (i + 3) % length, scratchCartesian4)
        Matrix2.Cartesian3.subtract(next, position, next)
        var bottom = Matrix2.Cartesian3.subtract(extrudedPosition, position, scratchCartesian3)

        normal = Matrix2.Cartesian3.normalize(Matrix2.Cartesian3.cross(bottom, next, normal), normal)
  ...
  function computeExtrudedEllipse(options) {
    ...
    topBoundingSphere.center = Matrix2.Cartesian3.add(center, scaledNormal, topBoundingSphere.center)
    topBoundingSphere.radius = semiMajorAxis

    scaledNormal = Matrix2.Cartesian3.multiplyByScalar(ellipsoid.geodeticSurfaceNormal(center, scaledNormal), options.extrudedHeight, scaledNormal)
    bottomBoundingSphere.center = Matrix2.Cartesian3.add(center, scaledNormal, bottomBoundingSphere.center)
  ...
  function computeRectangle(center, semiMajorAxis, semiMinorAxis, rotation, granularity, ellipsoid, result) {
    ...
    if (rectangle.width > ComponentDatatype.CesiumMath.PI) {
      rectangle.north = rectangle.north > 0.0 ? ComponentDatatype.CesiumMath.PI_OVER_TWO - ComponentDatatype.CesiumMath.EPSILON7 : rectangle.north
      rectangle.south = rectangle.south < 0.0 ? ComponentDatatype.CesiumMath.EPSILON7 - ComponentDatatype.CesiumMath.PI_OVER_TWO : rectangle.south
  ...
  function EllipseGeometry(options) {
    ...
    if (semiMajorAxis < semiMinorAxis) {
      throw new RuntimeError.DeveloperError('semiMajorAxis must be greater than or equal to the semiMinorAxis.')
    }
    if (granularity <= 0.0) {
   ...
   * @type {Number}
   */
  EllipseGeometry.packedLength = Matrix2.Cartesian3.packedLength + Matrix2.Ellipsoid.packedLength + VertexFormat.VertexFormat.packedLength + 9
...
//# sourceMappingURL=EllipseGeometry-0ca521d0.js.map
