define(['exports', './Matrix3-fa806b97', './defaultValue-fe22d8c0', './Check-6ede7e26', './Transforms-9052372a', './Math-dad82b4d'], function (
  exports,
  Matrix3,
  defaultValue,
  Check,
  ...
  QuadraticRealPolynomial.computeRealRoots = function (a, b, c) {
    ...
    const q = -0.5 * addWithCancellationCheck$1(b, Math$1.CesiumMath.sign(b) * Math.sqrt(radicand), Math$1.CesiumMath.EPSILON14)
    if (b > 0.0) {
      return [q / a, c / q]
    }
  ...
  function quadraticVectorExpression(A, b, c, x, w) {
    ...
    const l1 = w * (x * addWithCancellationCheck(A[Matrix3.Matrix3.COLUMN1ROW0], A[Matrix3.Matrix3.COLUMN0ROW1], Math$1.CesiumMath.EPSILON15) + b.y)
    const l0 = A[Matrix3.Matrix3.COLUMN0ROW0] * xSquared + A[Matrix3.Matrix3.COLUMN2ROW2] * wSquared + x * b.x + c

    const r1 = wSquared * addWithCancellationCheck(A[Matrix3.Matrix3.COLUMN2ROW1], A[Matrix3.Matrix3.COLUMN1ROW2], Math$1.CesiumMath.EPSILON15)
    const r0 = w * (x * addWithCancellationCheck(A[Matrix3.Matrix3.COLUMN2ROW0], A[Matrix3.Matrix3.COLUMN0ROW2]) + b.z)
  ...
  IntersectionTests.grazingAltitudeLocation = function (ray, ellipsoid) {
    ...
    const secondAxis = Matrix3.Cartesian3.normalize(Matrix3.Cartesian3.cross(reference, firstAxis, secondAxisScratch), secondAxisScratch)
    const thirdAxis = Matrix3.Cartesian3.normalize(Matrix3.Cartesian3.cross(firstAxis, secondAxis, thirdAxisScratch), thirdAxisScratch)
    const B = bScratch
    B[0] = firstAxis.x
    B[1] = firstAxis.y
    B[2] = firstAxis.z
    ...
    const temp = Matrix3.Matrix3.multiply(Matrix3.Matrix3.multiply(B_T, D, tempMatrix), C, tempMatrix)
    ...
    const solutions = quadraticVectorExpression(A, Matrix3.Cartesian3.negate(b, firstAxisScratch), 0.0, 0.0, 1.0)
    ...
    if (length > 0) {
      ...
      for (let i = 0; i < length; ++i) {
        ...
        const v = Matrix3.Cartesian3.normalize(Matrix3.Cartesian3.subtract(s, position, referenceScratch), referenceScratch)
      ...
      altitude =
        Matrix3.Cartesian3.magnitude(Matrix3.Cartesian3.subtract(closest, position, referenceScratch)) * Math.sqrt(1.0 - maximumValue * maximumValue)
