...
define([
  ...
  './Transforms-86b6fa28'
], function (exports, Matrix2, RuntimeError, when, WebGLConstants, Transforms) {
  'use strict'
    ...
    for (var property in geometry.attributes) {
      if (
        ...
        if (numberOfVertices !== num && numberOfVertices !== -1) {
          throw new RuntimeError.DeveloperError('All attribute lists must have the same number of attributes.')
        }
        //>>includeEnd('debug');
  ...
  var boundingRectanglePointsCartographicScratch = [new Matrix2.Cartographic(), new Matrix2.Cartographic(), new Matrix2.Cartographic()]
  ...
  Geometry._textureCoordinateRotationPoints = function (positions, stRotation, ellipsoid, boundingRectangle) {
    ...
    var enuCenter = Matrix2.Cartographic.toCartesian(rectangleCenter, ellipsoid, enuCenterScratch)
    ...
    var rotation = Transforms.Quaternion.fromAxisAngle(Matrix2.Cartesian3.UNIT_Z, -stRotation, enuRotationScratch)
    var textureMatrix = Matrix2.Matrix3.fromQuaternion(rotation, enuRotationMatrixScratch)
  ...
  function GeometryAttribute(options) {
    ...
    if (options.componentsPerAttribute < 1 || options.componentsPerAttribute > 4) {
      throw new RuntimeError.DeveloperError('options.componentsPerAttribute must be between 1 and 4.')
    }
