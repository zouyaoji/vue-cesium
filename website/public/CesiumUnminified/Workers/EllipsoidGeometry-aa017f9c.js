...
define([
  ...
  function EllipsoidGeometry(options) {
    ...
    if (slicePartitions < 3) {
      throw new RuntimeError.DeveloperError('options.slicePartitions cannot be less than three.')
    }
    if (stackPartitions < 3) {
      throw new RuntimeError.DeveloperError('options.stackPartitions cannot be less than three.')
    }
    //>>includeEnd('debug');
   ...
   * @type {Number}
   */
  EllipsoidGeometry.packedLength = 2 * Matrix2.Cartesian3.packedLength + VertexFormat.VertexFormat.packedLength + 7
  ...
  EllipsoidGeometry.createGeometry = function (ellipsoidGeometry) {
    ...
    slicePartitions = Math.round((slicePartitions * Math.abs(maximumClock - minimumClock)) / ComponentDatatype.CesiumMath.TWO_PI)
    stackPartitions = Math.round((stackPartitions * Math.abs(maximumCone - minimumCone)) / ComponentDatatype.CesiumMath.PI)
    ...
    if (vertexFormat.st || vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
      for (i = 0; i < vertexCount; i++) {
        ...
        if (vertexFormat.st) {
          ...
          st[stIndex++] = Math.atan2(normalST.y, normalST.x) / ComponentDatatype.CesiumMath.TWO_PI + 0.5
