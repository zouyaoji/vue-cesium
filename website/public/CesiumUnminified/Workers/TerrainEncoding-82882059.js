define([
  ...
  './Math-dad82b4d'
], function (exports, Transforms, Matrix3, Check, defaultValue, Matrix2, AttributeCompression, ComponentDatatype, Math$1) {
  'use strict'
  ...
  function isScaledSpacePointVisible(occludeeScaledSpacePosition, cameraPositionInScaledSpace, distanceToLimbInScaledSpaceSquared) {
    ...
    const isOccluded =
      vhMagnitudeSquared < 0
        ? vtDotVc > 0
        : vtDotVc > vhMagnitudeSquared && (vtDotVc * vtDotVc) / Matrix3.Cartesian3.magnitudeSquared(vt) > vhMagnitudeSquared
    return !isOccluded
  }
  ...
  function computeMagnitude(ellipsoid, position, scaledSpaceDirectionToPoint) {
    ...
    const direction = Matrix3.Cartesian3.divideByScalar(scaledSpacePosition, magnitude, directionScratch)
    ...
    const sinAlpha = Matrix3.Cartesian3.magnitude(Matrix3.Cartesian3.cross(direction, scaledSpaceDirectionToPoint, direction))
  ...
  TerrainEncoding.prototype.encode = function (vertexBuffer, bufferIndex, position, uv, height, normalToPack, webMercatorT, geodeticSurfaceNormal) {
    ...
    if (this.quantization === TerrainQuantization$1.BITS12) {
      ...
      const compressed0 = AttributeCompression.AttributeCompression.compressTextureCoordinates(cartesian2Scratch)

      Matrix2.Cartesian2.fromElements(position.z, h, cartesian2Scratch)
      const compressed1 = AttributeCompression.AttributeCompression.compressTextureCoordinates(cartesian2Scratch)

      Matrix2.Cartesian2.fromElements(u, v, cartesian2Scratch)
      const compressed2 = AttributeCompression.AttributeCompression.compressTextureCoordinates(cartesian2Scratch)
      ...
      if (this.hasWebMercatorT) {
        ...
        const compressed3 = AttributeCompression.AttributeCompression.compressTextureCoordinates(cartesian2Scratch)
  ...
  TerrainEncoding.prototype.decodePosition = function (buffer, index, result) {
    ...
    if (this.quantization === TerrainQuantization$1.BITS12) {
      const xy = AttributeCompression.AttributeCompression.decompressTextureCoordinates(buffer[index], cartesian2Scratch)
      result.x = xy.x
      result.y = xy.y

      const zh = AttributeCompression.AttributeCompression.decompressTextureCoordinates(buffer[index + 1], cartesian2Scratch)
      result.z = zh.x
  ...
  TerrainEncoding.prototype.getExaggeratedPosition = function (buffer, index, result) {
    ...
    if (hasExaggeration && this.hasGeodeticSurfaceNormals) {
      ...
      const heightDifference = TerrainExaggeration$1.getHeight(rawHeight, exaggeration, exaggerationRelativeHeight) - rawHeight
