define([
  ...
  CullingVolume.prototype.computeVisibility = function (boundingVolume) {
    ...
    for (let k = 0, len = planes.length; k < len; ++k) {
      const result = boundingVolume.intersectPlane(Plane.Plane.fromCartesian4(planes[k], scratchPlane))
  ...
  function FrustumGeometry(options) {
     ...
     */
    this.packedLength =
      2 + frustumPackedLength + Matrix3.Cartesian3.packedLength + Transforms.Quaternion.packedLength + VertexFormat.VertexFormat.packedLength
  }
  ...
  FrustumGeometry.createGeometry = function (frustumGeometry) {
    ...
    if (
      ...
      const normals = defaultValue.defined(vertexFormat.normal) ? new Float32Array(3 * 4 * numberOfPlanes) : undefined
      const tangents = defaultValue.defined(vertexFormat.tangent) ? new Float32Array(3 * 4 * numberOfPlanes) : undefined
      const bitangents = defaultValue.defined(vertexFormat.bitangent) ? new Float32Array(3 * 4 * numberOfPlanes) : undefined
      const st = defaultValue.defined(vertexFormat.st) ? new Float32Array(2 * 4 * numberOfPlanes) : undefined
