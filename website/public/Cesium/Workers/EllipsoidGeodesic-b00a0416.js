define(['exports', './Matrix3-b6f074fa', './defaultValue-0a909f67', './Math-e97915da'], function (t, a, i, n) {
  'use strict'
  ...
  function r(t, i, r, h) {
    a.Cartesian3.normalize(h.cartographicToCartesian(i, o), s),
      ...
      (function (t, a, i, s, o, r, h) {
        const d = (a - i) / a,
          c = r - s,
          u = Math.atan((1 - d) * Math.tan(o)),
          l = Math.atan((1 - d) * Math.tan(h)),
          M = Math.cos(u),
          g = Math.sin(u),
          _ = Math.cos(l),
          p = Math.sin(l),
        ...
        let v,
          ...
          U,
          A = c,
          b = n.CesiumMath.TWO_PI,
          w = Math.cos(A),
          R = Math.sin(A)
        do {
          ;(w = Math.cos(A)), (R = Math.sin(A))
          const t = m - C * w
          let a
          ;(S = Math.sqrt(_ * _ * R * R + t * t)),
            (O = H + f * w),
            (v = Math.atan2(S, O)),
            0 === S ? ((a = 0), (q = 1)) : ((a = (f * R) / S), (q = 1 - a * a)),
            (b = A),
            (U = O - (2 * H) / q),
            isFinite(U) || (U = 0),
            (A = c + e(d, a, q, v, S, O, U))
        } while (Math.abs(A - b) > n.CesiumMath.EPSILON12)
        const x = (q * (a * a - i * i)) / (i * i),
          ...
          D = Math.atan2(_ * R, m - C * w),
          T = Math.atan2(M * R, m * w - C)
      ...
      (function (t) {
        const a = t._uSquared,
          ...
          O = 1 - 5 * _,
          S =
            H * c - (C * Math.sin(2 * c) * _) / 2 - (v * Math.sin(4 * c) * p) / 16 - (O * Math.sin(6 * c) * f) / 48 - (5 * Math.sin(8 * c) * m) / 512,
          q = t._constants
        ;(q.a = i),
          (q.b = n),
          (q.f = e),
  ...
  Object.defineProperties(h.prototype, {
    ...
    (h.prototype.interpolateUsingSurfaceDistance = function (t, n) {
      ...
      let C =
        ...
        o *
          (1 -
            ...
            ((5 * m) / 4 - (115 * p) / 16) * h -
            (29 * p * d) / 16) +
        (f / 2 - H + (71 * m) / 32 - (85 * p) / 16) * c +
        ((5 * H) / 16 - (5 * m) / 4 + (383 * p) / 96) * u -
        g * ((m - (11 * p) / 2) * c + (5 * p * u) / 2) +
        ((29 * m) / 96 - (29 * p) / 16) * l +
        (539 * p * M) / 1536
      ...
      const S = Math.cos(2 * s.sigma + C),
        ...
        A = s.cosineU * U,
        b = s.sineU * q,
        w = Math.atan2(q * s.sineHeading, A - b * s.cosineHeading) - e(s.f, s.sineAlpha, s.cosineSquaredAlpha, C, q, U, S)
      return i.defined(n)
        ? ((n.longitude = this._start.longitude + w), (n.latitude = O), (n.height = 0), n)
        : new a.Cartographic(this._start.longitude + w, O, 0)
    }),
