 ...
 * @LastEditTime: 2021-10-28 21:28:02
...
/**
 * copy from packages/composables/lib to dist/composables
 */
function copyEsm() {
  return gulp.src(cjs + '/**').pipe(gulp.dest(path.resolve(distBundle, 'lib/composables')))
}

function copyCjs() {
  return gulp.src(esm + '/**').pipe(gulp.dest(path.resolve(distBundle, 'es/composables')))
}
