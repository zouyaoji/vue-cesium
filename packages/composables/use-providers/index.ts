...
export default function (props, ctx, vcInstance: VcComponentInternalInstance) {
  ...
  vcInstance.mount = async () => {
    const { imageryLayerViewModel } = $services
    ...
    if (props.projectionTransforms && props.projectionTransforms.from !== props.projectionTransforms.to) {
      const ignoreTransforms = vcInstance.proxy.$options.name === 'VcProviderImageryBaidumap' ||
    ...
    return imageryLayerViewModel && imageryLayerViewModel.__setProvider(imageryProvider)
  }
  vcInstance.unmount = async () => {
    const { imageryLayerViewModel } = $services
    return imageryLayerViewModel && imageryLayerViewModel.__setProvider(undefined)
  }
