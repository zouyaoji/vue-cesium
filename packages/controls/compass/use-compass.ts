...
export default function(props, { emit }, vcInstance: VcComponentInternalInstance) {
  ...
  const unsubscribeFromViewerChange: AnyFunction = undefined
  let unsubscribeFromPostRender: AnyFunction = undefined
  let unsubscribeFromClockTick: AnyFunction = undefined

  let orbitMouseMoveFunction: AnyFunction = undefined
  let orbitMouseUpFunction: AnyFunction = undefined
  let orbitTickFunction: AnyFunction = undefined
  ...
  let rotateMouseUpFunction: AnyFunction = undefined
  let rotateMouseMoveFunction: AnyFunction = undefined
  ...
  // state methods
  const viewerChange = (instance: VcComponentInternalInstance) => {
  ...
  const unload = async () => {
    ...
    unsubscribeFromViewerChange && unsubscribeFromViewerChange()
    return true
  }
