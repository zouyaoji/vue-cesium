...
export default function (props, { emit }, vcInstance: VcComponentInternalInstance, $services: VcViewerProvider) {
  ...
  let zoominTickFunction: AnyFunction
  let zoominMouseUpFunction: AnyFunction
  let isZoomin = false
  let zoominLastTimestamp = 0
  let unsubscribeFromClockTickZoomin: AnyFunction

  let zoomoutTickFunction: AnyFunction
  let zoomoutMouseUpFunction: AnyFunction
  let iszoomout = false
  let zoomoutLastTimestamp = 0
  let unsubscribeFromClockTickZoomout: AnyFunction

  let zoomBarScrollMouseMoveFunction: AnyFunction
  let zoomBarScrollMouseUpFunction: AnyFunction
  let zoombarTickFunction: AnyFunction
  let isZoomBarScrolling = false
  let unsubscribeFromClockTickZoomBar: AnyFunction
