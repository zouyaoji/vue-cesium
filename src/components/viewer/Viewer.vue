  ...
  watch: {
    selectionIndicator (val) {
      ...
      if (Cesium.defined(viewer.selectionIndicator) && !viewer.selectionIndicator.isDestroyed() && !val) {
        ...
        let selectionIndicatorContainer = getDocumentByClassName(
          ...
          'vc-viewer-selectionIndicatorContainer'
        )
        ...
        selectionIndicatorContainer.className = 'vc-viewer-selectionIndicatorContainer'
    ...
    infoBox (val) {
      ...
      if (Cesium.defined(viewer.infoBox) && !viewer.infoBox.isDestroyed() && !val) {
        ...
        let infoBoxContainer = getDocumentByClassName(viewerContainer.children, 'vc-viewer-infoBoxContainer')
        ...
        infoBoxContainer.className = 'vc-viewer-infoBoxContainer'
        ...
        viewer._forceResize = true
        viewer.resize()
      }
    },
    geocoder (val) {
      const { viewer, resizeToolbar, viewerContainer } = this
      let toolbar = getDocumentByClassName(viewerContainer.children, 'vc-viewer-toolbar')
      if (Cesium.defined(viewer.geocoder) && !viewer.geocoder.isDestroyed() && !val) {
        viewer.geocoder.destroy()
        viewer._geocoder = undefined
        let geocoderContainer = getDocumentByClassName(toolbar.children, 'vc-viewer-geocoderContainer')
        ...
        geocoderContainer.className = 'vc-viewer-geocoderContainer'
...
Either specify options.terrainProvider instead or set options.baseLayerPicker to true.`)
    ...
    animation (val) {
      ...
      if (Cesium.defined(viewer.animation) && !viewer.animation.isDestroyed() && !val) {
        ...
        let animationContainer = getDocumentByClassName(viewerContainer.children, 'vc-viewer-animationContainer')
        ...
        animationContainer.className = 'vc-viewer-animationContainer'
    ...
    timeline (val) {
      ...
      if (Cesium.defined(viewer.timeline) && !viewer.timeline.isDestroyed() && !val) {
        ...
        let timelineContainer = getDocumentByClassName(viewerContainer.children, 'vc-viewer-timelineContainer')
        ...
        timelineContainer.className = 'vc-viewer-timelineContainer'
    ...
    fullscreenButton (val) {
      ...
      if (Cesium.defined(viewer.fullscreenButton) && !viewer.fullscreenButton.isDestroyed() && !val) {
        ...
        let fullscreenContainer = getDocumentByClassName(viewerContainer.children, 'vc-viewer-fullscreenContainer')
        ...
        fullscreenContainer.className = 'vc-viewer-fullscreenContainer'
    ...
    vrButton (val) {
      ...
      if (Cesium.defined(viewer.vrButton) && !viewer.vrButton.isDestroyed() && !val) {
        ...
        let vrContainer = getDocumentByClassName(viewerContainer.children, 'vc-viewer-vrContainer')
        ...
        vrContainer.className = 'vc-viewer-vrContainer'
    ...
    navigation (val) {
      ...
      if (Cesium.defined(viewer.navigation) && !val) {
        ...
        let navigationContainer = getDocumentByClassName(viewerContainer.children, 'vc-viewer-navigationContainer')
        ...
        navigationContainer.className = 'vc-viewer-navigationContainer'
  ...
  methods: {
    ...
    resizeToolbar (parent, child) {
      Array.prototype.slice.call(parent.children).forEach((element) => {
        switch (element.className) {
          case 'vc-viewer-geocoderContainer':
    ...
    init () {
      ...
      //   }
      // }
      this.viewerContainer = getDocumentByClassName(this.$refs.viewer.children, 'vc-viewer')
    ...
    async getCesiumScript () {
      ...
      glob.document.body.appendChild($script)
      ...
      if (!glob.Cesium) {
        getCesiumScript().then((script) => {
          script.onload = () => {
            ...
            $link.rel = 'stylesheet'
            glob.document.head.appendChild($link)
            ...
            if (Cesium.SuperMapImageryProvider) {
              ...
              glob.document.body.appendChild($scriptZlib)
  ...
  destroyed () {
    if (glob.Cesium) {
      ...
      for (let script of scripts) {
        ...
        if (glob.Cesium.SuperMapImageryProvider) {
