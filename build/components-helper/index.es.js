...
function parse(options, file) {
    ...
    var subTitles = [];
    var _titleRegExp = new RegExp(titleRegExp);
    var titleContent = _file.match(_titleRegExp);
    if (titleContent && titleContent.length > 0) {
        var str = _file.substring(titleContent[0].length);
        var doFlag = true;
        while (doFlag) {
            var subTitleContent = str.match(_titleRegExp);
            if (subTitleContent && subTitleContent.length > 0) {
                ...
                str = str.substring(subTitleContent.index + subTitleContent[0].length);
            }
            else {
                doFlag = false;
            }
...
function writeFileRecursive(path, buffer) {
    ...
    mkdir(lastPath, { recursive: true }, function () {
