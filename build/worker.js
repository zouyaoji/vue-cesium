...
;(async () => {
  ...
  parentPort.once('message', async ({ port }) => {
    ...
    for (const sourceFile of sourceFiles) {
      messagePort.postMessage({
        type: 'log',
        message: chalk.yellow('Generating definition for file: ' + chalk.bold(sourceFile.getFilePath()))
      })
      ...
      for (const outputFile of emitOutput.getOutputFiles()) {
        ...
        messagePort.postMessage({
          type: 'log',
          message: chalk.green('Definition for file: ' + chalk.bold(sourceFile.getBaseName()) + ' generated')
        })
      }
