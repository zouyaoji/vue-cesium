...
const outputDir = path.resolve(buildOutput, './vue-cesium')
...
async function getComponents() {
  ...
  //
  return raw.filter(f => f !== 'package.json' && f !== 'index.ts').map(f => ({ path: path.resolve(compRoot, f), name: f }))
}
...
const pathsRewriter = id => {
  if (id.startsWith(`${VC_PREFIX}/components`)) return id.replace(`${VC_PREFIX}/components`, '..')
...
async function buildComponents() {
  ...
  const builds = componentPaths.map(async ({ path: p, name: componentName }) => {
    const entry = path.resolve(p, './index.ts')
    if (!fs.existsSync(entry)) return

    const external = id => {
      return id.startsWith(VUE_REGEX) || id.startsWith(VUE_MONO) || id.startsWith(VC_PREFIX) || externals.some(i => id.startsWith(i))
    }
    const esm = {
      format: 'es',
